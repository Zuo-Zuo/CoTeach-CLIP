...
def torch_center_and_normalize(points,p="inf"):
    ...
    if p != "fro" and p!= "no": scale = torch.max(torch.norm(points - center, p=float(p),dim=1))
    elif p=="fro" : scale = torch.norm(points - center, p=p )
    elif p=="no": scale = 1.0
...
class ShapeNet(data.Dataset):
    def __init__(self, partition='train', whole=False, num_points=1024):
        ...
        self.whole = whole
        with open(self.data_list_file, 'r') as f: lines = f.readlines()
        if self.whole:
            with open(test_data_list_file, 'r') as f: test_lines = f.readlines()
...
# =================================================================
# [MODIFIED] Class with JSON Description Support & Clean Data
# =================================================================
class ShapeNetRender(ShapeNet):
    _has_printed_debug = False # 类变量，用于控制只打印一次
    ...
    def __init__(self, partition='train', whole=False, num_points=1024):
        ...
        # [VERIFICATION] Load JSON
        self.json_path = './data/shapenet_1000.json'
        self.descriptions = {}
        ...
        if os.path.exists(self.json_path):
            try:
                ...
                # 打印加载成功信息
                print(f"[JSON SUCCESS] Loaded {len(self.descriptions)} descriptions from {self.json_path}")
            except Exception as e:
                print(f"[JSON ERROR] Failed to load JSON: {e}")
        else:
            print(f"[JSON WARNING] File not found at {self.json_path}. Will use fallback prompts.")

    def __getitem__(self, idx):
        sample = self.file_list[idx]
        points = IO.get(os.path.join(self.pc_path, sample['file_path'])).astype(np.float32)
        
        # [CLEAN DATA] No Jitter, No Rotation
        points = self.random_sample(points, self.sample_points_num)
        points = self.pc_norm(points)
        points = torch.from_numpy(points).float()
        ...
        # 查找描述
        if model_id in self.descriptions:
            raw_text = self.descriptions[model_id]
            # [DEBUG PRINT] 只在第一次命中时打印，证明用上了
            if not ShapeNetRender._has_printed_debug:
                print(f"\n[DEBUG CHECK] FOUND DESCRIPTION for {model_id}:")
                print(f"   -> \"{raw_text}\"")
                ShapeNetRender._has_printed_debug = True
        else:
            class_name = TAXONOMY_MAP.get(taxonomy_id, "object")
            raw_text = f"a 3d model of a {class_name}"
        
        # Tokenize: [77] -> [1, 77] (CLIP needs batch dim)
        # Note: We take [0] because tokenize returns a batch, we want a single item
        text_tokens = tokenize(raw_text, truncate=True)[0]

        return image, points, text_tokens, self.views_azim[rand_idx], self.views_elev[rand_idx], self.views_dist[rand_idx]
...
def collate_fn(batch):
    ...
    elif elem_type.__module__ == 'pytorch3d.structures.meshes': return batch
    elif isinstance(elem, dict): return batch
    ...
    elif isinstance(elem, (str, bytes)): return batch
    elif isinstance(elem, collections.abc.Mapping): return {key: collate_fn([d[key] for d in batch]) for key in elem}
    elif isinstance(elem, tuple) and hasattr(elem, '_fields'): return elem_type(*(collate_fn(samples) for samples in zip(*batch)))
